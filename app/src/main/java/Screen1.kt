...
val TwitterDarkYellow = Color(0xFFE8B807)
...
fun TweetingApp() {
    ...
    LaunchedEffect(Unit) {
        db.collection("tweets")
            ...
            .addSnapshotListener { snapshot, _ ->
                tweets = snapshot?.documents?.mapNotNull { it.toObject<Tweet>()?.copy(id = it.id) }
                    ?: emptyList()
            }
    }
    ...
    Column(
        ...
        val maxCharLimit = 20
        val charCount = tweetMessage.length


        Spacer(modifier = Modifier.height(4.dp))
        Text(
            text = if (maxCharLimit-charCount>=0) "${maxCharLimit - charCount} characters remaining" else "character limit exceeded by ${charCount-maxCharLimit} !",
            color = if (charCount > maxCharLimit) Color.Red else if(maxCharLimit-charCount<5)  TwitterDarkYellow else TwitterDarkGray,
            fontSize = 12.sp
        )
        ...
        Button(
            onClick = {
                if (tweetMessage.isNotBlank() && charCount <= maxCharLimit) {
